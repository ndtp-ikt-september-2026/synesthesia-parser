...
class ScrapingEngine:
    ...
    CATEGORY_PRESETS = {
        # Гитары и музыкальные инструменты
        'Музыкальные инструменты': 'https://pop-music.ru/catalog/gitaryi/',
        'Гитары': 'https://pop-music.ru/catalog/gitaryi/',
        ...
        'Гитарное оборудование': 'https://pop-music.ru/catalog/gitarnoe-oborudovanie/',
    ...
    # Canonical 5 guitar gear subcategories verified in the user's OpenCart shop
    GUITAR_GEAR_SHOP_SUBCATEGORIES = {
        'Комбики': 'https://pop-music.ru/catalog/gitarnoe-oborudovanie/kombiki/',
        'Усилители для гитар': 'https://pop-music.ru/catalog/gitarnoe-oborudovanie/usiliteli-dlya-gitar/',
        'Кабинеты': 'https://pop-music.ru/catalog/gitarnoe-oborudovanie/kabinety/',
        'Педали для гитар': 'https://pop-music.ru/catalog/gitarnoe-oborudovanie/pedali-dlya-gitar/',
    ...
    def _get_session(self) -> requests.Session:
        if self._session is None:
            ...
            self._session.headers.update({
                ...
                'Sec-Ch-Ua': f'"{chr(34)}Chromium{chr(34)}";v="128", "Not;A=Brand";v="24", "Google Chrome";v="128"',
                'Sec-Ch-Ua-Mobile': '?0',
    ...
    def parse_product_detail(self, product_url: str, category_name: str) -> Optional[InstrumentPayload]:
        ...
        # Products under 'Гитарное оборудование' MUST strictly belong to a shop subcategory.
        if not category_ids or (root == CategoryResolver.ROOT_GUITAR_GEAR and (not subcat or len(category_ids) < 2)):
    ...
    def scrape(
        ...
        # 2. Handle root 'Гитарное оборудование' category:
        # Route strictly through the 5 subcategories that exist in the user's shop
        clean_target = target.strip().lower()
        clean_resolved = resolved_url.rstrip('/')
        is_guitar_gear_root = (
            clean_target in ['гитарное оборудование', 'гитарное-оборудование', 'guitar gear', 'guitar-gear'] or
            clean_resolved == f'{self.BASE_URL}/catalog/gitarnoe-oborudovanie'
        )

        if is_guitar_gear_root:
            self._log('Распознан запрос на категорию "Гитарное оборудование".')
            self._log(f'Сбор производится исключительно по 5 подкатегориям магазина: {list(self.GUITAR_GEAR_SHOP_SUBCATEGORIES.keys())}')
            return self._scrape_guitar_gear_subcategories(limit=limit)

        # 3. Handle standard catalog category listing with pagination
    ...
    def _scrape_guitar_gear_subcategories(self, limit: int) -> List[InstrumentPayload]:
        '''
        Scrapes items strictly from the 5 guitar gear subcategories present in the shop.
        Distributes requested limit evenly across subcategories.
        '''
        results: List[InstrumentPayload] = []
        seen_models = set()
        subcats = list(self.GUITAR_GEAR_SHOP_SUBCATEGORIES.items())
        per_subcat = max(1, limit // len(subcats))

        # Pass 1: Gather per_subcat items from each shop subcategory
        for subcat_name, subcat_url in subcats:
            if len(results) >= limit or self._is_stopped:
                break
            target_count = min(len(results) + per_subcat, limit)
            page = 1
            while len(results) < target_count and not self._is_stopped:
                product_links = self.fetch_listing_page(subcat_url, page=page)
                if not product_links or self._is_stopped:
                    break
                added_in_page = 0
                for link in product_links:
                    if len(results) >= target_count or self._is_stopped:
                        break
                    if not link.startswith('https://pop-music.ru/products/'):
                        continue
                    self._log(f'[{len(results) + 1}/{limit}] ({subcat_name}) Парсинг карточки: {link}')
                    item = self.parse_product_detail(link, category_name=subcat_name)
                    if self._is_stopped:
                        break
                    if item and item.model not in seen_models:
                        seen_models.add(item.model)
                        results.append(item)
                        added_in_page += 1
                        self._log(f'✓ Успешно извлечен [{subcat_name}]: {item.name} (SKU: {item.model}, Цена: {item.price:,.0f} BYN)')
                        if self.on_progress:
                            self.on_progress(len(results), limit, item)
                    elif item:
                        self._log(f'Дубликат SKU {item.model}, пропуск.')
        ...
        # Pass 2: If limit is not yet met, gather remaining items from available subcategories
        if len(results) < limit and not self._is_stopped:
            for subcat_name, subcat_url in subcats:
                if len(results) >= limit or self._is_stopped:
                    break
                page = 1
                while len(results) < limit and not self._is_stopped:
                    ...
                    for link in product_links:
                        ...
                        if not link.startswith('https://pop-music.ru/products/'):
                            continue
                        self._log(f'[{len(results) + 1}/{limit}] ({subcat_name} добор) Парсинг карточки: {link}')
                        item = self.parse_product_detail(link, category_name=subcat_name)
                        if self._is_stopped:
                            break
                        if item and item.model not in seen_models:
        ...
        self._log(f'Завершен сбор данных по подкатегориям гитарного оборудования! Собрано товаров: {len(results)}')
        return results
